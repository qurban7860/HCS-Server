...
const { TicketInvestigationReason } = require('../models');
...
exports.patchTicketInvestigationReason = async (req, res, next) => {
  try{
    ...
    const result = await this.dbservice.patchObject(TicketInvestigationReason, req.params.id, getDocFromReq(req));
...
exports.deleteTicketInvestigationReason = async (req, res, next) => {
  try{
    ...
    return res.status(StatusCodes.BAD_REQUEST).send("Ticket investigation reason deleted successfully!");
  } catch( error ){
    logger.error(new Error(error));
    return res.status(StatusCodes.BAD_REQUEST).send( error?.message );
  }

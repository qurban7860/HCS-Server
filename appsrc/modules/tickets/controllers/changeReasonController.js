...
const { TicketChangeReason } = require('../models');
...
exports.patchTicketChangeReason = async (req, res, next) => {
  try{
    ...
    const result = await this.dbservice.patchObject(TicketChangeReason, req.params.id, getDocFromReq(req));
    return res.status(StatusCodes.ACCEPTED).send("Ticket change reason updated successfully!");
  } catch( error ){
    logger.error(new Error(error));
    return res.status(StatusCodes.BAD_REQUEST).send( error?.message );
  }
...
exports.deleteTicketChangeReason = async (req, res, next) => {
  try{
    ...
    return res.status(StatusCodes.BAD_REQUEST).send("Ticket change reason deleted successfully!");
  } catch( error ){
    logger.error(new Error(error));
    return res.status(StatusCodes.BAD_REQUEST).send( error?.message );
  }

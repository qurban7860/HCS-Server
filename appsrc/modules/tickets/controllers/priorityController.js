...
exports.getTicketCountByPriority = async (req, res, next) => {
  try {
...
exports.getTicketPriority = async (req, res, next) => {
  try {
    const result = await this.dbservice.getObjectById(TicketPriority, this.fields, req.params.id, this.populate);
    return res.status(StatusCodes.OK).json(result);
  } catch (error) {
    logger.error(new Error(error));
    return res.status(StatusCodes.BAD_REQUEST).send(error?.message);
...
exports.getTicketPriorities = async (req, res, next) => {
  try {
    ...
    const result = await this.dbservice.getObjectList(req, TicketPriority, this.fields, this.query, this.orderBy, this.populate);
    return res.status(StatusCodes.OK).json(result);
  } catch (error) {
    logger.error(new Error(error));
    return res.status(StatusCodes.BAD_REQUEST).send(error?.message);
...
exports.searchTicketPriorities = async (req, res, next) => {
  try {
...
exports.patchTicketPriority = async (req, res, next) => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      logger.error(new Error(errors));
